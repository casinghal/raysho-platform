...
export const metadata: Metadata = {
  title: 'Privacy Policy — FintelliQ',
  description: 'Privacy Policy for FintelliQ. How we collect, use, and protect your data.',
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.wrap}>
        ...
        <div style={s.note}>
          FintelliQ currently operates as a free-access, no-signup platform. We collect minimal data. This policy explains what data is collected, by whom, and how it is used.
        </div>

        <h2 style={s.h2}>1. Who We Are</h2>
        <p style={s.p}>FintelliQ is an independent educational platform for Finance and Accounting professionals. References to "we", "us", or "FintelliQ" in this policy refer to the operator of fintelliQ.com.</p>

        <h2 style={s.h2}>2. Data We Collect</h2>
        <p style={s.p}><strong style={{ color: '#eef0f4' }}>Platform access (no account required):</strong> FintelliQ does not currently require account creation or login for platform access. We do not collect your name, email address, or personal details during normal platform use.</p>
        ...
        <p style={s.p}><strong style={{ color: '#eef0f4' }}>Technical data collected automatically by hosting infrastructure:</strong> Like all websites, our hosting provider (Vercel) may log IP addresses, browser type, and page requests for operational and security purposes. This data is governed by Vercel's own privacy policy (vercel.com/legal/privacy-policy) and is not used by FintelliQ for tracking or profiling.</p>
        <p style={s.p}><strong style={{ color: '#eef0f4' }}>When paid subscriptions are introduced:</strong> We will collect email address, payment information (processed by Stripe — not stored by FintelliQ), and subscription status. This policy will be updated at that time with full details.</p>

        <h2 style={s.h2}>3. Third-Party Services</h2>
        <p style={s.p}>FintelliQ uses the following third-party services which may process data in connection with platform operation:</p>
